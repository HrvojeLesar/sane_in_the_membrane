...
void CMDns::CPartialQueryResult::set_ipv4(std::string&& addr) {
    m_ip_v4_addr = std::move(addr);
}
void CMDns::CPartialQueryResult::set_ipv6(std::string&& addr) {
    m_ip_v6_addr = std::move(addr);
}
void CMDns::CPartialQueryResult::set_port(uint16_t port) {
    m_port = port;
}

std::optional<SQueryResult> CMDns::CPartialQueryResult::build_ipv4() {
...
std::optional<SQueryResult> CMDns::CPartialQueryResult::build_ipv6() {
...
void CMDns::CPartialQueryResult::reset() {
    m_ip_v4_addr = std::nullopt;
    m_ip_v6_addr = std::nullopt;
    m_port       = std::nullopt;
}

CMDns::CMDns(long discover_timeout_sec, long query_timeout_sec) :
    m_options({
        .discover_timeout_sec    = discover_timeout_sec,
        .query_timeout_sec       = query_timeout_sec,
        .continue_after_discover = true,
        .continue_after_query    = true,
    }) {}

CMDns::CMDns(SmDNSOptions& options) : m_options(options) {}

std::expected<void, std::string> CMDns::discover_services(void) {
    int sockets[32];
    ...
    do {
        ...
        if (res > 0) {
            ...
            if (m_options.continue_after_discover)
                break;
        }
    } while (res > 0);
...
std::expected<std::vector<SQueryResult>, std::string> CMDns::query_services() {
...
int CMDns::open_client_sockets(int* sockets, int max_sockets, int port) {
    int num_sockets = 0;
...
int CMDns::discover_callback(int sock, const struct sockaddr* from, size_t addrlen, mdns_entry_type_t entry, uint16_t query_id, uint16_t rtype, uint16_t rclass, uint32_t ttl,
...
int CMDns::query_callback(int sock, const struct sockaddr* from, size_t addrlen, mdns_entry_type_t entry, uint16_t query_id, uint16_t rtype, uint16_t rclass, uint32_t ttl,
                          const void* data, size_t size, size_t name_offset, size_t name_length, size_t record_offset, size_t record_length) {
...
std::expected<void, std::string> CMDns::send_mdns_query(mdns_query_t* query, size_t count) {
    int sockets[32];
    int query_id[32];
    int num_sockets = open_client_sockets(sockets, sizeof(sockets) / sizeof(sockets[0]), 0);
    if (num_sockets <= 0) {
    ...
    do {
        struct timeval timeout;
        timeout.tv_sec  = m_options.query_timeout_sec;
        timeout.tv_usec = 0;
        ...
        if (res > 0) {
            ...
            if (m_options.continue_after_query)
                break;
        }
    } while (res > 0);
...
mdns_string_t CMDns::ipv4_address_to_string(char* buffer, size_t capacity, const struct sockaddr_in* addr, size_t addrlen) {
...
mdns_string_t CMDns::ipv6_address_to_string(char* buffer, size_t capacity, const struct sockaddr_in6* addr, size_t addrlen) {
    char host[NI_MAXHOST]    = {0};
    char service[NI_MAXSERV] = {0};
    int  ret                 = getnameinfo((const struct sockaddr*)addr, (socklen_t)addrlen, host, NI_MAXHOST, service, NI_MAXSERV, NI_NUMERICSERV | NI_NUMERICHOST);
    int  len                 = 0;
...
mdns_string_t CMDns::ip_address_to_string(char* buffer, size_t capacity, const struct sockaddr* addr, size_t addrlen) {
