    ...
    class CAvahiService {
      ...
      public:
        CAvahiService(int port = 50051) : m_simple_poll(avahi_simple_poll_new()), m_port(port) {
            log::debug("Create avahi");
        }

        ~CAvahiService() {
            log::debug("Destroy avahi");
            if (m_client)
        ...
        std::expected<void, int> start_and_wait() {
            log::debug("Starting Avahi service registration");
            int error{};
            ...
            if (error < 0) {
                log::error("Avahi service registration failed with errorno: {}", error);
      ...
      private:
        void create_service() {
            ...
            if (avahi_entry_group_is_empty(m_group)) {
                log::info("Adding service '{}'", m_service_name);
                ...
                if (ret != AVAHI_OK) {
                    log::error("Failed to add {} service to avahi: {}", m_service_name, avahi_strerror(ret));
                    return shutdown();
                }
        ...
        void client_callback(AvahiClient* c, AvahiClientState state) {
            ...
            switch (state) {
                ...
                case AVAHI_CLIENT_FAILURE: {
                    log::error("Client failed with error: {}", avahi_strerror(avahi_client_errno(m_client)));
                    return shutdown();
                }
        ...
        void group_callback(AvahiEntryGroup* g, AvahiEntryGroupState state) {
            ...
            switch (state) {
                case AVAHI_ENTRY_GROUP_ESTABLISHED: log::info("Service '{}' successfully established on port '{}'", m_service_name, m_port); break;

                case AVAHI_ENTRY_GROUP_COLLISION: {
                    alternative_service_name();
                    log::warn("Service name collision, renaming service to '{}'", m_service_name);
                ...
                case AVAHI_ENTRY_GROUP_FAILURE: {
                    log::error("Entry group failed with error: {}", avahi_strerror(avahi_client_errno(m_client)));
                    return shutdown();
                }
      ...
      public:
        ...
        void start(std::optional<int> port) {
            ...
            while (!m_shutdown.load(std::memory_order::relaxed)) {
                reset();
                log::debug("Autorestrtable started");
